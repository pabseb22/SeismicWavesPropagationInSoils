...
source nu0.txt
# soil gammaref hyperbolic model (-)
...
puts "Number Of elements TOTAL:$numTotalEle"
...
# Definir el numero de curvas de degradacion



    #if {{$number == 1 && $name == "hello"} || {$number == 0&&$name == "yes"}} {...}
    
    source curvasdeg.txt
...
wipeAnalysis
#RECORDER JUAN PABLO
recorder Node -file AcelJP.out -time -dT $motionDT -node $numTotalNode  -dof 1   accel
# record nodal displacments, velocities, and accelerations at each time step
...
#recorder Element –ele 5 -file back.out -dT $motionDT material 2 backbone  25 50 100
